 ...
 */
public class MdsKey {
  private static final byte[] VALUE_ROW_PREFIX = {'v'}; // value row to store metadata value
  private static final byte[] INDEX_ROW_PREFIX = {'i'}; // index row used for metadata search

  public static String getMetadataKey(String type, byte[] rowKey) {
    MDSKey.Splitter keySplitter = new MDSKey(rowKey).split();
    // The rowkey is [rowPrefix][targetType][targetId][key] for value rows and
   ...
   * [{@link #VALUE_ROW_PREFIX}][targetType][targetId][key] for value index rowss
   */
  ...
  private static MDSKey.Builder getMDSKeyPrefix(Id.NamespacedId targetId, byte[] indexRowPrefix) {
    String targetType = KeyHelper.getTargetType(targetId);
    MDSKey.Builder builder = new MDSKey.Builder();
    builder.add(indexRowPrefix);
